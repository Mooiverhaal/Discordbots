...
exports.start = function(SETUP) {
  ...
  const UpdateEmbed = function() {
    let dot = TICK_N % 2 === 0 ? 'Glanerbrug' : 'Roleplay';
    let embed = new Discord.RichEmbed()
    .setAuthor("Glanerbrug Roleplay™ Server Status", "https://cdn.discordapp.com/attachments/799751262664917042/802794434961342504/logo.png")
    .setColor(0x2894C2)
    .setFooter(TICK_N % 2 === 0 ? '⚪ Glanerbrug Roleplay™' : '⚫ Glanerbrug Roleplay™')
    .setTimestamp(new Date())
    .addField('\n\u200b\nHoe kan je de server joinen?','Je kan de server joinen doormiddel van **mqdd5a** in te voeren bij Direct Connect. Onderaan staat de server status om te kijken hoeveel mensen er online zijn en in de wachtrij staan.\n\u200b\n',false)
    if (STATUS !== undefined)
    {
  ...
  bot.on('ready',() => {
    ...
    // bot.user.setGame('Glanerbrug', 'https://www.twitch.tv/shroud');
    bot.user.setActivity('Glanerbrug Roleplay™',{'url':'https://www.twitch.tv/shroud','type':'STREAMING'});
  ...
  bot.on('message', message => {
    if (message.content.startsWith('Xzeg')) {
          message.delete();
